...
def _is_nv(val: Any) -> bool:
    """True if *val* is the Nv sentinel."""
    return val is NV
...
class DequeueEngine:
    ...
    def size(self, name: str) -> int:
        ...
        grid = self._get(name)
        return max(len(grid), 2) * self._DEFAULT_WIDTH
    ...
    def space(self, name: str) -> int:
        ...
        return self.size(name) - self.count(name)
    ...
    def _fill_empty(self, name: str, value: Any, position: str) -> None:
        ...
        """
        grid = self._get(name)
        # Collect indices of all user-empty cells first, then Nv cells.
        empty_cells: list[tuple[int, int]] = []
        nv_cells: list[tuple[int, int]] = []
        for row_idx, row in enumerate(grid):
            for col_idx in range(len(row)):
                if row[col_idx] is EMPTY:
                    empty_cells.append((row_idx, col_idx))
                elif row[col_idx] is NV:
                    nv_cells.append((row_idx, col_idx))

        if position in ("first", "last"):
            candidates = empty_cells if empty_cells else nv_cells
        elif position in ("sFirst", "bLast"):
            candidates = empty_cells if len(empty_cells) >= 2 else nv_cells
        else:
