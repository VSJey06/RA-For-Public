...
class Runtime:
    ...
    def __init__(self) -> None:
        ...
        self._var_types: dict[str, str] = {}  # name -> "I" | "S" | "TF"
    ...
    def execute(self, program_node: ProgramNode) -> None:
        ...
        # Validate PF dependencies whenever PF is active and pH/fF appear.
        ph_registered = bool(self._pf_engine.ph_entries)
        ff_registered = bool(self._pf_engine.ff_nodes)
        if self._pf_activated and (ph_registered or ff_registered):
            try:
    ...
    def execute_node(self, node: Node) -> None:
        ...
        if isinstance(node, DbNode):
            ...
            self._execute_len(node)
        elif isinstance(node, StringTransformNode):
            self._execute_string_transform(node)
        elif isinstance(node, CharNode):
            self._execute_char(node)
        elif isinstance(node, CharMethodNode):
            self._execute_char_method(node)
        elif isinstance(node, RoundNode):
            self._execute_round(node)
        elif isinstance(node, IsNode):
            self._execute_is(node)
        elif isinstance(node, AbsNode):
            self._execute_abs(node)
        elif isinstance(node, TypeInfoNode):
            self._execute_type_info(node)
    ...
    def evaluate(self, node: Node) -> Any:
        ...
        if isinstance(node, LenNode):
            return self._compute_len(node)
        if isinstance(node, StringTransformNode):
            return self._compute_string_transform(node)
        if isinstance(node, RoundNode):
            return self._compute_round(node)
        if isinstance(node, IsNode):
            return self._compute_is(node)
        if isinstance(node, AbsNode):
            return self._compute_abs(node)
        if isinstance(node, TypeInfoNode):
            return self._compute_type_info(node)
        if isinstance(node, CharNode):
            return self._compute_char(node)
        if isinstance(node, CharMethodNode):
            return self._compute_char_method(node)
        raise RuntimeError(f"Node type not implemented: {type(node).__name__}")
    ...
    def _execute_assignment(self, node: AssignmentNode) -> None:
        ...
        if node.is_declaration:
            self._var_types[node.name] = node.type_name
        if self._active_db is not None:
            self.db_engine.set_value(self._active_db, node.name, value)

    # ── Type info (.type:variable) ───────────────────────────────────────

    def _compute_type_info(self, node: TypeInfoNode) -> str:
        """Compute the RA type name for a variable."""
        name = node.name
        if name not in self._var_types and name not in self.global_scope:
            raise RuntimeError(f"Unknown variable '{name}'")
        return self._var_types.get(name, "~")

    def _execute_type_info(self, node: TypeInfoNode) -> None:
        t = self._compute_type_info(node)
        print(t)
        self.global_scope["_"] = t

    # ── Len (.len:variable) ──────────────────────────────────────────────

    def _compute_len(self, node: LenNode) -> int:
        """Compute the character count of a String variable."""
        name = node.name
        if name not in self._var_types and name not in self.global_scope:
            raise RuntimeError(f"Unknown variable '{name}'")
        t = self._var_types.get(name)
        if t is not None and t != "S":
            raise RuntimeError(f"'.len:' only supports String (S) datatype, not '{t}'")
        value = self.global_scope.get(name)
        if not isinstance(value, str):
            raise RuntimeError(
                f"'.len:' expected String value for '{name}', got {type(value).__name__}"
            )
        return len(value)

    def _execute_len(self, node: LenNode) -> None:
        result = self._compute_len(node)
        print(result)
        self.global_scope["_"] = result

    # ── Abs (.abs:variable) ─────────────────────────────────────────────

    def _compute_abs(self, node: AbsNode) -> int:
        """Compute the absolute value of a numeric (I) variable."""
        name = node.name
        if name not in self._var_types and name not in self.global_scope:
            raise RuntimeError(f"Unknown variable '{name}'")
        t = self._var_types.get(name)
        if t is not None and t != "I":
            raise RuntimeError(
                f"'.abs:' only supports Integer (I) datatype, not '{t}'"
            )
        value = self.global_scope.get(name)
        if not isinstance(value, (int, float)):
            raise RuntimeError(
                f"'.abs:' expected numeric value for '{name}', "
                f"got {type(value).__name__}"
            )
        return abs(value)

    def _execute_abs(self, node: AbsNode) -> None:
        result = self._compute_abs(node)
        print(result)
        self.global_scope["_"] = result

    # ── Round (.round:variable) ─────────────────────────────────────────

    def _compute_round(self, node: RoundNode) -> int:
        """Compute the nearest whole number of a numeric (I) variable."""
        name = node.name
        if name not in self._var_types and name not in self.global_scope:
            raise RuntimeError(f"Unknown variable '{name}'")
        t = self._var_types.get(name)
        if t is not None and t != "I":
            raise RuntimeError(
                f"'.round:' only supports Integer (I) datatype, not '{t}'"
            )
        value = self.global_scope.get(name)
        if not isinstance(value, (int, float)):
            raise RuntimeError(
                f"'.round:' expected numeric value for '{name}', "
                f"got {type(value).__name__}"
            )
        return round(value)

    def _execute_round(self, node: RoundNode) -> None:
        result = self._compute_round(node)
        print(result)
        self.global_scope["_"] = result

    # ── Is (.is:variable) ───────────────────────────────────────────────

    def _compute_is(self, node: IsNode) -> bool:
        """Return the boolean state of a TF variable."""
        name = node.name
        if name not in self._var_types and name not in self.global_scope:
            raise RuntimeError(f"Unknown variable '{name}'")
        t = self._var_types.get(name)
        if t is not None and t != "TF":
            raise RuntimeError(
                f"'.is:' only supports TF datatype, not '{t}'"
            )
        value = self.global_scope.get(name)
        return bool(value)

    def _execute_is(self, node: IsNode) -> None:
        result = self._compute_is(node)
        print(result)
        self.global_scope["_"] = result

    # ── String transform (.upper / .lower / .trim) ─────────────────────

    def _compute_string_transform(self, node: StringTransformNode) -> str:
        """Compute .upper:variable, .lower:variable, or .trim:variable."""
        name = node.name
        method = node.method
        if name not in self._var_types and name not in self.global_scope:
            raise RuntimeError(f"Unknown variable '{name}'")
        ...
        if t is not None and t != "S":
            raise RuntimeError(
                f"'.{method}:' only supports String (S) datatype, not '{t}'"
            )
        value = self.global_scope.get(name)
        if not isinstance(value, str):
            raise RuntimeError(
                f"'.{method}:' expected String value for '{name}', "
                f"got {type(value).__name__}"
            )
        transform = {"upper": str.upper, "lower": str.lower, "trim": str.strip}
        return transform[method](value)

    def _execute_string_transform(self, node: StringTransformNode) -> None:
        result = self._compute_string_transform(node)
        print(result)
        self.global_scope["_"] = result

    # ── Char (.char:variable,index) ─────────────────────────────────────

    def _compute_char(self, node: CharNode) -> str:
        """Compute the char at index for a String variable."""
        name = node.name
        idx = node.index
        if name not in self._var_types and name not in self.global_scope:
            raise RuntimeError(f"Unknown variable '{name}'")
        ...
        if t is not None and t != "S":
            raise RuntimeError(
                f"'.char:' only supports String (S) datatype, not '{t}'"
            )
        value = self.global_scope.get(name)
        if not isinstance(value, str):
            raise RuntimeError(
                f"'.char:' expected String value for '{name}', "
                f"got {type(value).__name__}"
            )
        if not value:
            raise RuntimeError("String is empty")
        if idx < 0 or idx >= len(value):
            raise RuntimeError(
                f"Index out of range: {idx} (string length {len(value)})"
            )
        return value[idx]

    def _execute_char(self, node: CharNode) -> None:
        result = self._compute_char(node)
        print(result)
        self.global_scope["_"] = result

    # ── Char methods (.first / .last / .count / .find / .replace) ─────

    def _compute_char_method(self, node: CharMethodNode) -> Any:
        """Compute the result of a Char method operation."""
        m = node.method
        if node.name not in self._var_types and node.name not in self.global_scope:
            raise RuntimeError(f"Unknown variable '{node.name}'")
        t = self._var_types.get(node.name)
        if t is not None and t != "S":
            raise RuntimeError(
                f"'.{m}:' only supports String (S) datatype, not '{t}'"
            )
        value = self.global_scope.get(node.name)
        if not isinstance(value, str):
            raise RuntimeError(
                f"'.{m}:' expected String value for '{node.name}', "
                f"got {type(value).__name__}"
            )
        if m == "first":
            if not value:
            ...
            return value.count(node.arg)
        elif m == "find":
            return value.find(node.arg)
        elif m == "replace":
            return value.replace(node.arg, node.arg2)
        else:
            raise RuntimeError(f"Unknown char method '{m}'")

    def _execute_char_method(self, node: CharMethodNode) -> None:
        result = self._compute_char_method(node)
        print(result)
        self.global_scope["_"] = result
            ...
            try:
                if owner == "stack":
                    if node.property == "pop":
                        self.global_scope["_"] = self.stack_engine.pop(name)
                    elif node.property == "peek":
                        self.global_scope["_"] = self.stack_engine.peek(name)
                    else:
                        self.global_scope["_"] = self.evaluate(node)
                    return
                if owner == "queue":
                    if node.property == "pop":
                        self.global_scope["_"] = self.queue_engine.pop(name)
                    elif node.property == "peek":
                        self.global_scope["_"] = self.queue_engine.peek(name)
                    else:
                        self.global_scope["_"] = self.evaluate(node)
                    return
                if owner == "dequeue":
